...
import com.lakecat.web.http.BaseConstant;
...
public class HttpUtil {
    ...
    private static RequestConfig requestConfig = RequestConfig.custom()
            ...
            .setConnectionRequestTimeout(10000000)
            .build();
    ...
    private static BaseResponse<String> execute(CloseableHttpClient closeableHttpClient, HttpUriRequest request, String encode) {
        ...
        try {
            ...
            log.error("failed to http execute: " + e.toString());
            return BaseResponse.error(BaseResponseCodeEnum.HTTP_ERR, e.getMessage());
        } finally {
            try {
        ...
        try {  //关闭连接、释放资源
            ...
            log.error("failed to close http: " + e.toString());
            return BaseResponse.error(BaseResponseCodeEnum.HTTP_CLOSE_ERR, e.getMessage());
        }
        ...
        switch (statusCode) {
            ...
            case 204:
                if (map != null && map.size() != 0 && map.get(BaseConstant.RES_CODE) != null && map.get(BaseConstant.RES_MESSAGE) != null) {
                    instance = BaseResponse.getInstance(BaseResponseCodeEnum.SUCCESS.getCode(), BaseResponseCodeEnum.SUCCESS.name(), BaseResponseCodeEnum.SUCCESS.getMessage(), map.get(BaseConstant.RES_DATA));
                } else {
                    instance = BaseResponse.getInstance(BaseResponseCodeEnum.SUCCESS.getCode(), BaseResponseCodeEnum.SUCCESS.name(), BaseResponseCodeEnum.SUCCESS.getMessage(), content);
                }
                break;
            case 202:
                map.put("statusCode", "202");
                instance = BaseResponse.getInstance(BaseResponseCodeEnum.SUCCESS.getCode(),  BaseResponseCodeEnum.SUCCESS.name(), BaseResponseCodeEnum.SUCCESS.getMessage(), JSON.toJSONString(map));
                break;
            case 400:
                instance = BaseResponse.getInstance(BaseResponseCodeEnum.REQUEST_ILLEGAL.getCode(), BaseResponseCodeEnum.REQUEST_ILLEGAL.getMessage(), JSON.toJSONString(map));
                break;
            default:
                instance = BaseResponse.getInstance(BaseResponseCodeEnum.SYS_UNA.getCode(), BaseResponseCodeEnum.SYS_UNA.getMessage(), JSON.toJSONString(map));
                break;
        }
        return instance;
    }
