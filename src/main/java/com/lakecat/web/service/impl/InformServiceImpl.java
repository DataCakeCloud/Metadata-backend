...
//        list.clear();
        if (!InfTraceContextHolder.gcp()){
            ...
            //发起钉钉提醒
            req.setTaskUrl("https://xxx/task/detail?id=" + id + "&name=" + name);
            req.setTableUrl(String.format("https://xxx/lakecat/meta-detail?id=%s&region=%s&databaseName=%s&tableName=%s", search.getId(), region, dbName, tableName));
