...
public class HttpUtil {
    ...
    public static BaseResponse doPost(String url, Map <String, String> params, String jarFile,
    ...
    private static BaseResponse <String> execute(CloseableHttpClient closeableHttpClient, HttpUriRequest request,
                                                 String encode) {
    ...
    private static BaseResponse setResponse(CloseableHttpResponse httpResponse, String encode) throws IOException {
        ...
        switch (statusCode) {
            case 200:
            case 204:
                instance = BaseResponse.getInstance(BaseResponseCodeEnum.SUCCESS.name(),
                        BaseResponseCodeEnum.SUCCESS.getMessage(), content);

                break;
            case 202:
                map.put("statusCode", "202");
                instance = BaseResponse.getInstance(BaseResponseCodeEnum.SUCCESS.name(),
                        BaseResponseCodeEnum.SUCCESS.getMessage(), JSON.toJSONString(map));
                break;
            case 400:
                instance = BaseResponse.getInstance(BaseResponseCodeEnum.REQUEST_ILLEGAL.name(),
                        BaseResponseCodeEnum.REQUEST_ILLEGAL.getMessage(), JSON.toJSONString(map));
                break;
            default:
                instance = BaseResponse.getInstance(BaseResponseCodeEnum.SYS_UNA.name(),
                        BaseResponseCodeEnum.SYS_UNA.getMessage(), JSON.toJSONString(map));
                break;
        }
        return instance;
    }

    private static BaseResponse urlRequestBase(HttpRequestBase httpbase, String url, Map <String, String> params,
                                               Map <String, String> headers, String encode) {
        if (encode == null) {
            encode = "utf-8";
        }
